...
export const useWebRTC = (roomId, user) => {
  ...
  useEffect(() => {
    ...
    return () => {
      // leaving room
      localMediaStream.current.getTrack().forEach((track) => track.stop());
  ...
  useEffect(() => {
    const handleNewPeer = async ({ peerId, createOffer, user: remoteUser }) => {
      ...
      connections.current[peerId] = new RTCPeerConnection({
        iceServers: freeice(), // to know ip address
      });

      // handle new ice condidate
      connections.current[peerId].oniceCondidate = (event) => {
